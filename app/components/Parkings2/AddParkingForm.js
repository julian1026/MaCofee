...
export default function AddParkingForm(props) {
    ...
    const [farmName, setParkingName] = useState("");
    ...
    const addParking = () => {
        ...
        if (!farmName || !addres || !description || !mt2 || !totalArboles) {
            toastRef.current.show("Todos los campos del formulario son obligatorios", 1000)
        } else if (size(imageSelected) === 0) {
            toastRef.current.show("La finca debe de tener almenos una foto", 1000)
        } else if (!ubicationParking) {
            toastRef.current.show("Tienes que localizar la finca en el mapa", 1000)
        } else {
            setIsloading(true);
    ...
    return (
        ...
        <ScrollView style={styles.scrollView} >
            ...
            <FormAdd
                ...
                setMt2={setMt2}
                setTotalArboles={setTotalArboles}
    ...
    return (
        <View style={styles.viewPhoto} >
            <Image
                ...
                style={{ width: (widthScreen - 150), height: 200 }}
            />
        </View>
    )
}
...
function FormAdd(props) {
    const { setDescription, setParkingName, setAddres, setIsVisibleMap, ubicationParking, setMt2, setTotalArboles } = props;
    return (
        <View style={styles.viewForm} >
            <Input
                placeholder="Nombre del predio o finca"
                containerStyle={styles.input}
                onChange={e => setParkingName(e.nativeEvent.text)}
            />
            <Input
                placeholder="nombre de la veredad"
                containerStyle={styles.input}
            ...
            />
            <Input
                placeholder="mt2 del  predio"
                containerStyle={styles.input}
                onChange={e => setMt2(e.nativeEvent.text)}
            />
            <Input
                placeholder="total arboles de cafe"
                containerStyle={styles.input}
                onChange={e => setTotalArboles(e.nativeEvent.text)}
            />
        </View>
    );
}
